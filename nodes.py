...
class TektiteVideoCombiner7:
    ...
    def combine_videos(
        ...
        **kwargs,
        ...
        while time.time() < deadline:
            ...
            raise ValueError(
                f"Tektite Video Combiner 7.0 timed out after {wait_timeout_sec}s while waiting. Last state: {last_state}"
            )
        print(
            f"[Tektite Video Combiner 7.0] Ready {expected_clips}/{expected_clips}. Starting normalize+stitch."
        )
    ...
    def _build_progress_line(
        ...
        *,
        ...
        return (
            f"[Tektite Video Combiner 7.0] Clips ready: {ready_clips}/{expected_clips} | "
    ...
    def _extract_paths_from_value(self, value: Any, _depth: int = 0, _seen: Optional[set] = None) -> List[str]:
        ...
        if hasattr(value, "get_stream_source"):
            try:
                ...
                print(f"[Tektite Video Combiner 7.0] get_stream_source failed: {type(exc).__name__}: {exc}")
    ...
    def _ensure_writable_output_path(self, resolved_output: str) -> str:
        ...
        print(
            f"[Tektite Video Combiner 7.0] Output directory not writable: {out_dir}. "
...
NODE_CLASS_MAPPINGS = {
    "TektiteVideoCombiner7": TektiteVideoCombiner7,
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "TektiteVideoCombiner7": "Tektite Video Combiner 7.0",
}
